...
#define PREC -6
	...
	if (debug){
		printf("send_packet:Timestamp=%lu.%.6lu,data[0]=%d,data[1]=%d,data[2]=%d,data[10]=%d,data[11]=%d\n", now.tv_sec,now.tv_usec,data[0],data[1],data[2],data[10],data[11]); 
		fflush(stdout);
	}
}
...
#undef Data
	...
	if (set_clock) {   /* you'd better be root, or ntpclient will crash! */
		...
		// ccz +:add timezone offset; note the timezone
		tv_set.tv_sec  += time_of_offset;
		// ccz +:use timezone
	//	struct timezone tz_set;
	//	tz_set.tz_minuteswest = time_of_offset/60;
	//	if (settimeofday(&tv_set,&tz_set)<0) {
...
void stuff_net_addr(struct in_addr *p, char *hostname)
{
	...
	if (debug){
		printf("stuff_net_addr:ntpsvr name=%s,IP Address=%s\n", ntpserver->h_name,inet_ntoa(*((struct in_addr *)ntpserver->h_addr))); 
		fflush(stdout);
	}
	if (ntpserver == NULL) {
		herror(hostname);
...
void setup_receive(int usd, short port)
{
...
int primary_loop(int usd, int num_probes, int interval, int goodness)
{
	...
	for (;;) {
		...
		if (debug){
			printf("selected some thing=%d, time[%lu.%.6lu],times=%d\n",i,to.tv_sec,to.tv_usec,probes_sent);
			fflush(stdout);
		}
		if ((i!=1)||(!FD_ISSET(usd,&fds))) {
			if (i==EINTR){perror("selected EINTR");	continue;	}
			if (i<0) perror("select");
			if (i==0) perror("select timeout");
			if (to.tv_sec == 0){// probe again
				if (probes_sent >= num_probes &&	num_probes != 0) {
					if (debug){
						printf("failed to send times(%d) data to the ntp server!exit!\n", num_probes);
						fflush(stdout);
					}
					return 1;
				}
		...
		pack_len=recvfrom(usd,incoming,sizeof_incoming,0,(struct sockaddr *)&sa_xmit,(size_t *)&sa_xmit_len);
		if (debug){
			printf("recvfrom some thing=%d,lengh=%d\n",pack_len,sa_xmit_len);
			fflush(stdout);
		}
		error = goodness+1;
		if (pack_len<0) {
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		c = getopt( argc, argv, "c:" DEBUG_OPTION "g:h:i:lp:rso:");// ccz +:0
		if (c == EOF) break;
		switch (c) {
			// ccz +:
			case 'o':
				time_of_offset = atoi(optarg);
				break;
			case 'c':
	...
	if (debug) {
		...
		live, udp_local_port, set_clock,time_of_offset);
	}
	...
	int ret = primary_loop(usd, probe_count, cycle_time, goodness);
