...
static int get_jpeg_stream(void)
{
	...
	rval = ioctl(fd_iav, IAV_IOC_READ_BITSTREAM_EX, &jpg_bs_info);
	if (rval < 0) {
...
//					printf("r=%d",r);
					...
					{
						histogram_stat_t tmp_histogram_stat;
					//	printf("Get recv, and begin to send info...\n");
						img_dsp_get_statistics(fd_iav,& aaa_tile_get_info, aaa_ae_info, aaa_awb_info, aaa_af_info, &aaa_hist_info,&act_tile,aaa_af_info2, &tmp_histogram_stat);				
					//	printf("info_mode =%d\n",info_mode);
						if(info_mode ==1)
						{
				...
				switch(data.layer2)
				{
						...
						switch(data.layer3)
						{
							...
							case ColorCorrection:
								...
								recv(new_fd,matrix,17536,0);
							   	...
							   	img_dsp_load_color_correction_table((u32)reg, (u32)matrix);
								img_dsp_enable_color_correction(fd_iav, 1);
							...
							case ToneCurve:
								...
								{
							/*	if(dataArray[0]==1||dataArray[0]==4)
									tone_curve.tone_curve_red[i]= dataArray[i];
								if(dataArray[0]==2||dataArray[0]==4)
									tone_curve.tone_curve_green[i]= dataArray[i];
								if(dataArray[0]==3||dataArray[0]==4)
					...
					//				printf("%d,%d,%d,%d,\n",cfa_noise_filter.enable,cfa_noise_filter.iso_center_weight_red,cfa_noise_filter.direct_thresh_k1_blue );
								...
								case SharpeningControl:
									...
									for(k = 0; k<256; k++){
										coring.coring[k]= dataArray[k+7];
									}
									luma_high_freq_noise_reduction_strength = dataArray[263];

									spatial_filter_info.isotropic_strength = dataArray[264];
									spatial_filter_info.directional_strength = dataArray[265];
									spatial_filter_info.edge_threshold = dataArray[266];

									retain_level = dataArray[267];

									max_change.max_change_down = dataArray[268];
									max_change.max_change_up = dataArray[269];

									sharpen_setting_min.low = dataArray[270];
									sharpen_setting_min.low_delta = dataArray[271];
									sharpen_setting_min.low_strength = dataArray[272];
									sharpen_setting_min.mid_strength = dataArray[273];
									sharpen_setting_min.high = dataArray[274];
									sharpen_setting_min.high_delta = dataArray[275];
									sharpen_setting_min.high_strength = dataArray[276];

									sharpen_setting_overall.low =dataArray[277];
									sharpen_setting_overall.low_delta = dataArray[278];
									sharpen_setting_overall.low_strength = dataArray[279];
									sharpen_setting_overall.mid_strength = dataArray[280];
									sharpen_setting_overall.high = dataArray[281];
									sharpen_setting_overall.high_delta = dataArray[282];
									sharpen_setting_overall.high_strength = dataArray[283];
									...
									img_dsp_set_sharpening_level_min(fd_iav,&sharpen_setting_min);
									img_dsp_set_sharpening_level_overall(fd_iav,&sharpen_setting_overall);
								...
								case MCTFControl:
									...
									mctf_info.threshold_1 = dataArray[2];
									mctf_info.threshold_2 = dataArray[3];
									mctf_info.y_max_change = dataArray[4];
									mctf_info.u_max_change = dataArray[5];
									mctf_info.v_max_change = dataArray[6];
									mctf_info.padding = 0;
						...
						switch(data.layer3)
							{
								...
								case AETileConfiguration:
									...
									img_dsp_get_config_statistics_info(fd_iav, &aaa_statistic_config);
								...
								case AFStatisticSetupEx:
									...
									img_dsp_config_statistics_info_ex(fd_iav,&af_statistic_setup_ex);
						...
						switch(data.layer3)
							...
							//		printf("%d,%d,%d\n",chroma_median_setup.enable,chroma_median_setup.cb_str,chroma_median_setup.cb_str);
								...
								case SharpeningControl:
									...
									img_dsp_get_spatial_filter(& spatial_filter_info);

									img_dsp_get_sharpening_fir( &fir,  & mode);

									luma_high_freq_noise_reduction_strength=img_dsp_get_luma_high_freq_noise_reduction();

									img_dsp_get_sharpening_fir_coring(&coring, & retain_level);

									img_dsp_get_sharpening_max_change( &max_change);

									img_dsp_get_sharpening_level_min( &sharpen_setting_min);
									img_dsp_get_sharpening_level_overall(&sharpen_setting_overall);
									...
									for(i=0;i<256;i++)
										sendBuffer[7+i]=coring.coring[i];
									sendBuffer[263]=luma_high_freq_noise_reduction_strength;

									sendBuffer[264]=spatial_filter_info.isotropic_strength;
									sendBuffer[265]=spatial_filter_info.directional_strength;
									sendBuffer[266]=spatial_filter_info.edge_threshold;
									sendBuffer[267]=retain_level;
									sendBuffer[268]=max_change.max_change_down;
									sendBuffer[269]=max_change.max_change_up;

									sendBuffer[270]=sharpen_setting_min.low;
									sendBuffer[271]=sharpen_setting_min.low_delta;
									sendBuffer[272]=sharpen_setting_min.low_strength;
									sendBuffer[273]=sharpen_setting_min.mid_strength;
									sendBuffer[274]=sharpen_setting_min.high;
									sendBuffer[275]=sharpen_setting_min.high_delta;
									sendBuffer[276]=sharpen_setting_min.high_strength;


									sendBuffer[277]=sharpen_setting_overall.low;
									sendBuffer[278]=sharpen_setting_overall.low_delta;
									sendBuffer[279]=sharpen_setting_overall.low_strength;
									sendBuffer[280]=sharpen_setting_overall.mid_strength;
									sendBuffer[281]=sharpen_setting_overall.high;
									sendBuffer[282]=sharpen_setting_overall.high_delta;
									sendBuffer[283]=sharpen_setting_overall.high_strength;
								...
								case MCTFControl:
									...
									sendBuffer[2]=mctf_info.threshold_1;
									sendBuffer[3]=mctf_info.threshold_2;
									sendBuffer[4]=mctf_info.y_max_change;
									sendBuffer[5]=mctf_info.u_max_change;
									sendBuffer[6]=mctf_info.v_max_change;
									char sendMCTF[10];
									for(i=0;i<7;i++)
									{
										int k;
						...
						switch(data.layer3)
							...
							//		printf("%d,%d,%d,%d\n",sendCharArray[0],sendCharArray[1],sendCharArray[2],sendCharArray[3]);
								...
								case AFStatisticSetupEx:
									...
									img_dsp_get_config_statistics_info_ex(&af_statistic_setup_ex);
