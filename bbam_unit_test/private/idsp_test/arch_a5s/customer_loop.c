...
void cus_main_loop(void* arg)
{
	...
	while(1) {
		...
		if(img_dsp_get_statistics(fd_iav, &tile_info, ae_info, awb_info, af_info, &hist_info, &act_tile, af_info_yuv,&tmp_histogram_stat))
			continue;
		//AE API
		...
		if(adj_pipe_ctl.cc_matrix_update) {
			img_dsp_enable_video_cc_table(fd_iav, adj_pipe_ctl.color_corr.matrix_3d_table_addr);
		...
		if(adj_pipe_ctl.sharp_level_minimum_update) {
			img_dsp_set_sharpening_level_min(fd_iav, &adj_pipe_ctl.sharp_level_minimum);
			adj_pipe_ctl.sharp_level_minimum_update = 0;
		}
		if(adj_pipe_ctl.sharp_level_overall_update) {
			img_dsp_set_sharpening_level_overall(fd_iav, &adj_pipe_ctl.sharp_level_overall);
		...
		if(adj_pipe_ctl.sharp_max_change_update) {
			img_dsp_set_sharpening_max_change(fd_iav, &adj_pipe_ctl.sharp_max_change);
			adj_pipe_ctl.sharp_max_change_update = 0;
		}
		if(adj_pipe_ctl.sharp_fir_update) {
			img_dsp_set_sharpening_fir(fd_iav, &adj_pipe_ctl.sharp_fir, 0);
			adj_pipe_ctl.sharp_fir_update = 0;
		}
		if(adj_pipe_ctl.sharp_coring_update) {
			img_dsp_set_sharpening_fir_coring(fd_iav, &adj_pipe_ctl.sharp_coring, adj_pipe_ctl.sharp_retain);
...
int main(int argc, char **argv)
{
	...
	img_dsp_config_statistics_info_ex(fd_iav, &af_eng_cof);
