...
static int capture(snd_pcm_uframes_t chunk_size)
{
	...
	//while (rest > 0) {
        ...
        if(count > 100) {
            ret = -1;
            //printf("\n####chl %s:%d, detect audio capture is empty!!!!!\n", __FILE__, __LINE__);
        }
        else {
            ret = 0;
            //printf("\n####chl %s:%d, detect audio capture is ok...\n", __FILE__, __LINE__);
        }

		//rest -= c;
        //printf("####chl %s:%d, name:%s, chunk_bytes:%d, chunk_size:%d, c:%d\n", __FILE__, __LINE__, name, chunk_bytes, chunk_size, c);
	//}
...
int main(int argc, char *argv[])
{
    ...
    argc = 6;
    argv[0] = (char *)"test_audio_dev";
    argv[1] = (char *)"-C";
    argv[2] = (char *)"-fmu_law";
    argv[3] = (char *)"-c1";
    argv[4] = (char *)"-s0";
	argv[5] = (char *)"-r8000";
    argv[6] = (char *)"/tmp/audio_test_dev.dat";
