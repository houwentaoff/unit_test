...
static const hint_t hint[] = {
	...
	{ "", "\tset OSD width" },
	{ "", "\tset OSD height" },
	{ "0~3", "set font type, 0 Verdana, 1 Lucida, 2 Chinese(gbsn), 3 Korean(UnPen)" },
	{ "", "\tset font size " },
	...
	for (i = 0; i < MAX_ENCODE_STREAM_NUM; i++) {
		if (stream_text_info[i].enable) {
			...
			if (stream_text_info[i].rotate & SET_BIT(ROTATE_BIT)) {
				stream_text_info[i].win_width = format.encode_height;
				stream_text_info[i].win_height = format.encode_width;
			} else {
				stream_text_info[i].win_width = format.encode_width;
				stream_text_info[i].win_height = format.encode_height;
			}
		}
	}
	return 0;
...
{
	iav_mmap_info_t overlay_mem_info;
	if (ioctl(fd_overlay, IAV_IOC_MAP_OVERLAY, &overlay_mem_info) < 0) {
		perror("IAV_IOC_MAP_OVERLAY");
		return -1;
	}
	printf("\noverlay: start = 0x%p, total size = 0x%x ( bytes)\n",
	       overlay_mem_info.addr, overlay_mem_info.length);
	overlay_clut_addr = overlay_mem_info.addr + OVERLAY_CLUT_OFFSET;
	overlay_yuv_addr = overlay_mem_info.addr + OVERLAY_YUV_OFFSET;
	overlay_yuv_size = (overlay_mem_info.length - OVERLAY_YUV_OFFSET)
		...
		if (stream_text_info[i].enable) {
			...
			for (j = 0, total_size = 0; j < MAX_OVERLAY_AREA_NUM; j++) {
				...
				if (box->enable) {
					...
					area->clut_id = (box->outline == 0 ? box->color :
							box->color + CLUT_OUTLINE_OFFSET);
...
static int text_string_convert(int stream_id, int area_id)
{
	...
	bitmap_info_t bitmap_info;
	wchar_t *p;
	overlay_insert_area_ex_t * area = &overlay_insert[stream_id].area[area_id];
	textbox_t *box = &stream_text_info[stream_id].textbox[area_id];
