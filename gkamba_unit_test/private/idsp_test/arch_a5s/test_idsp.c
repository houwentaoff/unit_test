...
#define	CONTRAST					40
#define	AUTO_CONTRAST				41
#define	AUTO_CONTRAST_STR			42
...
static struct option long_options[] = {
	...
	{"auto-knee", HAS_ARG, 0, AUTO_KNEE},
	{0, 0, 0, 0},
};

u8 auto_knee_flag = 0, auto_knee_value = 128;
u8 color_contrast_flg = 0, color_contrast_str = 0;
...
u8 cc_flag,anti_aliasing_flag,anti_aliasing_strength;
//u8 zoom_flag;
//float zoom_factor;
...
static const struct hint_s hint[] = {
	...
	{"","\t\t\tset original and referenced wb gain for wb calibration"},
	{"","\t\twb calibration threshold"},
	{"","\t\t\ttransfer vignette gain table to awb statistics gain table"},
	{"","\t\t\tawb speed"},
	{"","\t\textra brightness"},
	{"","\t\toperation mode for sensor AR0331"},
	{"0~128","\t\tcolor contrast"},
	{"0 |1","\tauto color contrast open/close"},
	{"0~128","auto color contrast strength"},
	{"0~255","\t\tauto knee"},
	{0, 0},
};
...
//			break;
		...
		case HDR_OP_MODE:
			...
			break;
		case CONTRAST:
			color_contrast_flg = 1;
			color_contrast_str = atoi(optarg);
			break;
		case AUTO_CONTRAST:
			auto_contrast_flg = 1;
...
int main(int argc, char **argv)
{
	...
	if(sharpeness_flag){
		if (sharpeness_strength == 1){
			...
			img_dsp_set_sharpen_level_min(fd_iav, IMG_VIDEO, &sharpen_setting);
			...
			sharpen_setting.high_strength = 250;
			img_dsp_set_sharpen_level_overall(fd_iav, IMG_VIDEO, &sharpen_setting);
		}else{
			sharpen_setting.low = 0;
			sharpen_setting.low_delta = 1;
...
//	}
	...
	if(sharpening_fir_flag){
		img_dsp_set_sharpen_fir(fd_iav, IMG_VIDEO, &fir); // mode == 0 for video mode
	}
	if(coring_table_flag){
		img_dsp_set_sharpen_coring(fd_iav, IMG_VIDEO, &coring);
	}
	if(max_change_flag){
		img_dsp_set_sharpen_max_change(fd_iav, IMG_VIDEO, &max_change);
	}
	if(vig_to_awb_flag){
...
//		}
		...
		img_leave_still_capture(fd_iav);
//		save_jpeg_stream();
		/*input*/
...
*/
	...
	if(color_contrast_flg){
		img_set_color_contrast(color_contrast_str);
	}
	...
	if(auto_knee_flag){
		img_set_hlc_enable(0);
		img_set_ae_auto_knee((u8)auto_knee_value);
	}
	if(start_aaa)
	{
		aaa_api_t custom_aaa_api = {0};
