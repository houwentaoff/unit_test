...
static MDIN_ERROR_t MDIN3xx_SetSrcVideoPort(PMDIN_VIDEO_INFO pINFO, WORD nID)
{
	...
	PMDIN_OUTVIDEO_INFO pOUT_x = (PMDIN_OUTVIDEO_INFO)&pINFO->stOUT_x;
	// set attb[H/V-Polarity,ScanType], H/V size
	if (pOUT_x->frmt == VIDOUT_1920x1200pRB)
		memcpy(&pSRC->stATTB, (PBYTE)&defMDINSrcVideo_960[pSRC->frmt], sizeof(MDIN_SRCVIDEO_ATTB));
	else
...
static MDIN_ERROR_t MDIN3xx_SetOutVideoFrmt(PMDIN_VIDEO_INFO pINFO)
{
	...
	// set attb[H/V-Polarity,ScanType], H/V size
	if (pOUT_x->frmt == VIDOUT_1920x1200pRB)
		memcpy(&pOUT->stATTB, (PBYTE)&defMDINOutVideo_960[pOUT->frmt], sizeof(MDIN_OUTVIDEO_ATTB));
	else
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_SetOut4CH_OutVsync_Half(BOOL OnOff) // modified on 24Oct2011
{
	WORD rVal = 0;

	if (MDINHIF_RegRead(MDIN_HOST_ID, 0x030, &rVal)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(MDIN_HOST_ID, 0x030, rVal*2)) return MDIN_I2C_ERROR; // half frequency (vsync: 60 to 30Hz, 50 to 25Hz) for codec

#if defined(IN_960H_MODE)
	// input 960H mode
	if (OnOff)
		if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x249, 12,  2, 1)) return MDIN_I2C_ERROR; // set c intra mode (to reduce b/w)
#endif
